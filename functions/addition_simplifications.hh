...
template <class... Funcs> auto Add(Funcs... fs) {
  return Simplify(AddRaw(fs...));
}

template <class... Functions>
struct Simplification<Addition<Functions...>, /*var_add*/ 4> {
  typedef decltype(SimplifyV<Addition>(std::declval<tuple<Functions...> >(),
                                       std::true_type{})) type;

  static type Combine(Addition<Functions...> a) {
    SIMPLIFY_INFO("Simplifying Addition\n");
    return SimplifyV<Addition>(a.GetFunctions(), std::true_type{});
  }
};
...
template <class F>
struct Simplification<Addition<F, UnaryMinus<F> >, /*add_nadd*/ 2,
...
template <class F>
struct Simplification<Addition<UnaryMinus<F>, F>, /*nadd_add*/ 1,
                      typename std::enable_if<is_stateless<F>::value>::type> {
  typedef Zero type;
