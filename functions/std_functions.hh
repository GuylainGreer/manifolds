...
#define STD_FUNCTION(classname, funcname, index)                               \
  struct classname : Function<int_<index>, 1, 1>, FunctionCommon<classname> {  \
...
struct Pow : Function<int_<23>, 2, 1>, FunctionCommon<Pow> {
  static const bool stateless = true;
  static const ComplexOutputBehaviour complex_output = SameAsInput;
  template <class T, class U> auto eval(T t, U u) const {
    using std::pow;
    return pow(t, u);
  }
};
