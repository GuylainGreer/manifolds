...
#define STD_STREAM2(Func, func)				\
    template <class V>					\
    static void Stream2(std::ostream & s, Func,V)	\
    {s << #func;}
...
#undef STD_STREAM2
	...
	Polynomial<
	  ...
	  int_<Seq::size()>>,
	Funcs...> c2 =
	insert_element<0>(remove_element<0>
			  (c.GetFunctions()), p);
      Stream2(s, c2, vn);
    }
			...
			VNamer vn)
    {
      PrintMultElem(s, get<0>(m.GetFunctions()), vn);
      s << " * ";
      Stream2(s, Multiplication<Funcs...>
