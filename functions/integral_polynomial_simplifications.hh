...
namespace manifolds
{
  ...
  struct Simplification<
    Addition<
    IntegralPolynomial<
      std::integer_sequence<T1,t1s...>>,
    IntegralPolynomial<
      std::integer_sequence<T2,t2s...>>>,/*add_ip_ip*/0>
  {
	...
	Tr, indices..., (padding - padding)...> type;
    };

    static const int padding1 =
      sizeof...(t1s) < sizeof...(t2s) ?
      sizeof...(t2s) - sizeof...(t1s) :
      0;
    ...
    static const int padding2 =
      ...
      0;
    typedef typename Extended<
      std::integer_sequence<T2, t2s...>,
      std::make_integer_sequence<
	Tr, padding2>>::type padded2;
    ...
    struct Add<
      std::integer_sequence<Tr, indices1...>,
      std::integer_sequence<Tr, indices2...>>
    {
      typedef std::integer_sequence<
	Tr, (indices1 + indices2)...> type;
    };

    typedef IntegralPolynomial<
      typename Add<
	padded1, padded2>::type> type;

    static type Combine(Addition<
			IntegralPolynomial<
			std::integer_sequence<T1,t1s...>>,
			IntegralPolynomial<
			std::integer_sequence<T2,t2s...>>>)
  ...
  struct Simplification<
    ...
    IntegralPolynomial<
      std::integer_sequence<T1,t1s...>>,
    IntegralPolynomial<
      std::integer_sequence<T2,t2s...>>>,/*mult_ip_ip*/0>
  {
    typedef long Tr;
    ...
    struct get_value<i, std::integer_sequence<T,ts...>>:
      nth<i, std::integral_constant<T, ts>...>::type{};

    template <int, Tr, class>
      struct set_value;
      ...
      struct set_value<i, t, std::integer_sequence<T,ts...>>
    {
      ...
      {
	typedef std::integer_sequence<
	  Tr, get_value<lefts, values>::value...,
	  t, get_value<rights+i+1, values>::value...> type;
      };

      typedef typename apply<
	std::make_integer_sequence<int,i>,
	std::make_integer_sequence<
	  int,sizeof...(ts)-i-1>>::type type;
    };
    ...
    struct Add
    {
      typedef typename set_value<
	i+j, get_value<i+j,OutSeq>::value +
    ...
    {
      static_assert(i+j == sizeof...(ts),
		    "Internal error");
      typedef std::integer_sequence<
	T, ts...,
	(Tr)get_value<i, inputs1>::value *
	(Tr)get_value<j, inputs2>::value> type;
    };
  ...
  struct Simplification<
    IntegralPolynomial<
    std::integer_sequence<T, ts...>>, /*ip*/0>
  {
    template <T t>
    ...
    struct pop
    {
      ...
      {
	typedef std::integer_sequence<
	  T, nth<is, ic<t2s>...>::type::value...> type;
      };

      typedef typename apply<
	std::make_index_sequence<
	sizeof...(t2s)-1>>::type type;
    };
    ...
    template <T ... touts>
    struct stopping_condition<
      std::integer_sequence<T, touts...>>
    {
      static const bool value =
	sizeof...(ts) == 1 ||
	last<ic<touts>...>::type::value != 0;
    };

    template <T ... touts>
    struct pop_if<std::integer_sequence<T,touts...>,
		  true>
    {
	...
	next,
	stopping_condition<next>::value>::type type;
    };
    ...
    /*var_p_ip*/1>
  {
    typedef Variadic<
      Polynomial<T1,O1>,
      Polynomial<T2, int_<sizeof...(t2s)>>> type;

    static type Combine(Variadic<Polynomial<T1,O1>,
			IntegralPolynomial<
			std::integer_sequence<T2,t2s...>>> v)
    {
      SIMPLIFY_INFO("Simplifying variadic of "
		    "polynomial and integral_polynomial\n");
      return {
	get<0>(v.GetFunctions()),
	get<1>(v.GetFunctions()).ToPoly()
	};
    }
  };
  ...
  template <template<class...>class Variadic,
	    ...
	    class T2, T2 ... t2s>
  struct Simplification<
    Variadic<IntegralPolynomial<
	       std::integer_sequence<T2,t2s...>>,
	     Polynomial<T1,O1>>, /*var_ip_p*/0>
  {
    typedef Variadic<
      Polynomial<T2, int_<sizeof...(t2s)>>,
      Polynomial<T1,O1>> type;

    static type Combine(Variadic<IntegralPolynomial<
			std::integer_sequence<T2,t2s...>>,
			Polynomial<T1,O1>> v)
    {
      SIMPLIFY_INFO("Simplifying variadic of "
      ...
      return {
	get<0>(v.GetFunctions()).ToPoly(),
	get<1>(v.GetFunctions())
	};
    }
  };
  ...
  struct Simplification<
    UnaryMinus<IntegralPolynomial<
		 std::integer_sequence<T, coeffs...>>>, 0>
  {
    ...
    typedef IntegralPolynomial<
      std::integer_sequence<
	CType, (-(CType)coeffs)...>> type;

    static type Combine(UnaryMinus<IntegralPolynomial<
			std::integer_sequence<T, coeffs...>>>)
    {
...
struct Simplification<
    Addition<T, IntegralPolynomial<
                    std::integer_sequence<U, us>>>,
    /*add_f_ip_1*/2>
{
    typedef Composition<
        IntegralPolynomial<
            std::integer_sequence<U, us, (U)1>>, T> type;

    static type Combine(Addition<T, IntegralPolynomial<
                    std::integer_sequence<U, us>>> a)
    {
        SIMPLIFY_INFO("Simplifying addition of constant "
                      "and another function");
        auto t = a.GetFunctions();
        return {IntegralPolynomial<
                std::integer_sequence<U, us, (U)1>>(),
                get<0>(t)};
    }
};
...
struct Simplification<
    Multiplication<T, IntegralPolynomial<
        std::integer_sequence<U, (U)1> > >,
    /*mult_f_ip_1*/0>
{
    static T Combine(Multiplication<
                         T, IntegralPolynomial<
                             std::integer_sequence<U, (U)1> > > m)
    {
