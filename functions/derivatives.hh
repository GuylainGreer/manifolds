...
namespace manifolds {
  ...
  struct DerivativeWrapper
  {
    ...
    static auto GroupDeriv(const Group<Funcs...> & g,
			   Variable<i> v,
			   std::integer_sequence<
			   std::size_t, is...>)
    {
      return GetGroup(DerivativeWrapper<iter+1>::
		      Derivative(get<is>(g.GetFunctions()), v)...);
    }

    template <class ... Funcs, int i>
    static auto Derivative(const Group<Funcs...> & g,
			   Variable<i> v)
    {
    ...
    {
      return 
	AddRaw(DerivativeWrapper<iter+1>::
			  ...
			  std::size_t, rights...>)
    {
      ...
      return MultiplyRaw(get<lefts>(t)...,
			 DerivativeWrapper<iter+1>::
    ...
    {
      return 
	AddRaw(TimesDerivHelper<indices>(a, v)...);
    }
    ...
    struct IPDeriv<
      ...
      std::integer_sequence<T, output...>, t>
    {
      typedef std::integer_sequence<T, output...> type;
    };

    template <class T, T next, T ... rest,
	      T ... output, int term_order>
    struct IPDeriv<
      std::integer_sequence<T, next, rest...>,
      std::integer_sequence<T, output...>,
      term_order>
    {
    ...
    template <class T, T f, T ... coeffs,
	      class = typename std::enable_if<
		(sizeof...(coeffs)> 1)>::type>
    static auto Derivative(IntegralPolynomial<
			   std::integer_sequence<
			   T,f, coeffs...>> ip,
			   Variable<0> v)
    {
      PRINT_DERIV(ip, v, "non-constant integral polynomial");
      IntegralPolynomial<
	typename IPDeriv<std::integer_sequence<T, coeffs...>,
    ...
    static auto Derivative(IntegralPolynomial<
			   std::integer_sequence<T,f>> ip,
			   Variable<0> v)
    {
      PRINT_DERIV(ip, v, "constant integral polynomial");
      return zero;
    }

    template <class Seq, int i>
    static auto Derivative(IntegralPolynomial<Seq> ip,
			   Variable<i> v)
    {
      PRINT_DERIV(ip, v, "non-constant integral polynomial");
      return zero;
