...
namespace manifolds {
template <class...> struct Composition;

template <class... Funcs>
struct Group : Function<list<int_<5>, typename Funcs::indices...>,
                        max<Funcs::input_dim...>::value, sizeof...(Funcs)> {
  ...
  template <class... Args> auto dispatch(std::true_type, Args... args) const {
    return compose(args...);
  }
  ...
  template <class... Args> auto operator()(Args... args) const {
    return dispatch(and_<is_function<Args>...>(), args...);
  }

  template <class... Args> auto eval(Args... args) const {
    return eval(std::index_sequence_for<Funcs...>(), args...);
  }

  auto GetFunctions() const { return functions; }
};

template <class... Functions> auto GetGroup(Functions... fs) {
