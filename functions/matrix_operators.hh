...
namespace manifolds {
  ...
  template <class T>
    auto MassageMultiplicationOutput(T t, int_<1>, int_<1>)
  {
    return t.Coeff(0,0);
  }
  ...
  template <class ... Types, std::size_t rows,
	    std::size_t cols, class CoeffType>
  auto operator*(const tuple<Types...> & t,
		 const Matrix<rows,cols,CoeffType> & m)
  {
    auto a = ToArray(t);
    return Matrix<sizeof...(Types), 1,
		  typename decltype(a)::value_type>(a) * m;
  }
  ...
  {
    auto a = ToArray(t);
    return m * Matrix<sizeof...(Types), 1,
		      typename decltype(a)::value_type>(a);
  }
}
