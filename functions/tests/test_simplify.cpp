...
BOOST_AUTO_TEST_CASE(simplify_test)
{
  ...
  auto ps = Sin() + Sin();
  BOOST_CHECK_EQUAL(get<0>(ps.GetFunctions()).
		    GetCoeffs()[0], 0);
  BOOST_CHECK_EQUAL(get<0>(ps.GetFunctions()).
		    GetCoeffs()[1], 2);

  auto ms = Sin() * Sin();
  BOOST_CHECK_EQUAL(get<0>(ms.GetFunctions()).
		    GetCoeffs()[0], 0);
  BOOST_CHECK_EQUAL(get<0>(ms.GetFunctions()).
		    GetCoeffs()[1], 0);
  BOOST_CHECK_EQUAL(get<0>(ms.GetFunctions()).
