...
auto MakeInput(Rand &r, Eng &e, std::integer_sequence<int, Ns...>) {
  return make_my_tuple(((void)Ns, r(e))...);
}

template <int N, class Eng, class Rand> auto MakeInput(Rand &r, Eng &e) {
  return MakeInput(r, e, std::make_integer_sequence<int, N>());
}
...
void ComparePoint(M m1, M m2, double tolerance, std::true_type) {
  for (std::size_t r = 0; r < M::num_rows; r++)
    for (std::size_t c = 0; c < M::num_cols; c++)
      BOOST_CHECK_CLOSE(m1.Coeff(r, c), m2.Coeff(r, c), tolerance);
}
...
template <class F> void ComparePoint(F f1, F f2, double tolerance) {
...
template <class Tuple> void PrintTuple(const Tuple &t) {
  std::cout << "(";
  PrintTupleIter(t);
  std::cout << ")\n";
}
...
void PointwiseEqual(F f, FRef fr, int num_points = pointwise_default_num_points,
