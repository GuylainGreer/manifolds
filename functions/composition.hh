...
template <class C, bool> struct Delay {
  C c;
  Delay(const C &c) : c(c) {}
  C Simplify() const { return c; }
};

template <class C> Delay<C, true> GetDelay(const C &c) {
  return { c };
}

template <int output_dim> struct ToTuple {
  ...
  auto eval(X x, std::integer_sequence<std::size_t, indices...>,
    ...
    static_assert(iter_rows == 1 || iter_cols == 1,
                  "I'm not sure what's going on here");
    return make_my_tuple(x.Coeff(indices * iter_rows, indices * iter_cols)...);
  }
  ...
  template <class X> auto operator()(X x) const {
    static const int tag =
        output_dim == 1 ? 1 : (2 * (has_coeff_function<X>::value) +
                               4 * (is_gettable<X>::value));
    return eval(x, std::make_index_sequence<output_dim>(), int_<tag>());
  }
};

template <ComplexOutputBehaviour... behaviours> struct ComposedBehaviour;

template <ComplexOutputBehaviour left, ComplexOutputBehaviour right>
struct BehaviourMerger : std::integral_constant<ComplexOutputBehaviour, right> {
  static_assert(left == SameAsInput, "Internal error");
};

template <ComplexOutputBehaviour right>
struct BehaviourMerger<AlwaysComplex,
                       right> : std::integral_constant<ComplexOutputBehaviour,
                                                       AlwaysComplex> {};

template <ComplexOutputBehaviour right>
struct BehaviourMerger<NeverComplex,
                       right> : std::integral_constant<ComplexOutputBehaviour,
                                                       NeverComplex> {};

template <ComplexOutputBehaviour curr, ComplexOutputBehaviour... nexts>
struct ComposedBehaviour<
    curr, nexts...> : BehaviourMerger<curr,
                                      ComposedBehaviour<nexts...>::value> {};

template <ComplexOutputBehaviour last>
struct ComposedBehaviour<last> : std::integral_constant<ComplexOutputBehaviour,
                                                        last> {};

template <class... Functions>
struct Composition : Function<list<int_<1>, typename Functions::indices...>,
  ...
  static_assert(sizeof...(Functions) > 0, "Cannot have empty composition!");
  Composition() {}
  Composition(const Functions &... functions) : functions(functions...) {}
  Composition(const tuple<Functions...> &f) : functions(f) {}
  ...
  using prepare = typename std::remove_reference<
      typename std::remove_cv<InnerFunc>::type>::type;

  // Return a composite function
  template <class InnerFunc, class = typename std::enable_if<
                                 is_function<InnerFunc>::value>::type>
  auto operator()(InnerFunc f) const {
    return GetDelay(Composition<Composition, prepare<InnerFunc> >(*this, f))
        .Simplify();
  }
  ...
  template <class... InnerFuncs,
            class = typename std::enable_if<
                (sizeof...(InnerFuncs) > 1) &&
                and_<is_function<InnerFuncs>...>::value>::type>
  ...
  void Print(std::ostream &s) { stream(s, int_<0>()); }
...
template <class... Functions>
template <class... Args>
Composition<Group<Functions...>, Args...>
Group<Functions...>::compose(Args... args) const {
