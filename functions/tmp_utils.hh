...
namespace manifolds
{
  ...
  template <class Arg>
  struct and_<Arg> : bool_<Arg::value>{};
  ...
  struct or_<Arg, Args...> :
    std::conditional<
    Arg::value, std::true_type, or_<Args...>>::type{};

  template <class Arg>
  struct or_<Arg> : bool_<Arg::value>{};
  ...
  struct are_same<
    ...
    typename std::enable_if<
      sizeof...(Arg1s) != sizeof...(Arg2s)>::type>:
  std::false_type{};
  ...
  struct are_same<
    ...
    typename std::enable_if<
      sizeof...(Arg1s) == sizeof...(Arg2s)>::type>:
    and_<typename std::is_same<Arg1s, Arg2s>::type...>{};
  ...
  struct seq_size<std::integer_sequence<T,ts...>> :
    int_<sizeof...(ts)>{};
...
#define FUNC_CHECK_DEF(mname, code)		\
  ...
  struct common_type
  {
    ...
    typedef typename std::conditional<
      (std::is_unsigned<std_type>::value &&
       or_<std::is_signed<Types>...>::value),
