...
namespace manifolds {
template <int order1, int order2, class CoeffType1, class CoeffType2>
struct Simplification<Addition<Polynomial<CoeffType1, int_<order1> >,
                               Polynomial<CoeffType2, int_<order2> > >,
                      /*add_p1_p2*/ 0> {
  static const int new_order = order1 > order2 ? order1 : order2;
  ...
  static auto Init(const T1 &t1, const T2 &t2, std::true_type) {
    typedef typename std::common_type<decltype(t1.GetCoeffs()[0]),
                                      decltype(t2.GetCoeffs()[0])>::type type;
    std::array<type, array_size<decltype(t1.GetCoeffs())>::value> result;
    std::copy(t1.GetCoeffs().begin(), t1.GetCoeffs().end(), result.begin());
    return result;
  }

  template <class T1, class T2>
  static auto Init(const T1 &t1, const T2 &t2, std::false_type) {
    typedef typename std::common_type<decltype(t1.GetCoeffs()[0]),
                                      decltype(t2.GetCoeffs()[0])>::type type;
    std::array<type, array_size<decltype(t2.GetCoeffs())>::value> result;
    std::copy(t2.GetCoeffs().begin(), t2.GetCoeffs().end(), result.begin());
    return result;
  }
...
template <int order1, int order2, class CoeffType1, class CoeffType2>
struct Simplification<Multiplication<Polynomial<CoeffType1, int_<order1> >,
                                     Polynomial<CoeffType2, int_<order2> > >,
                      /*mult_p1_p2*/ 0> {
  ...
  static type
  Combine(Multiplication<Polynomial<CoeffType1, int_<order1> >,
                         Polynomial<CoeffType2, int_<order2> > > p) {
...
template <int order1, int order2, class CoeffType1, class CoeffType2>
struct Simplification<Composition<Polynomial<CoeffType1, int_<order1> >,
                                  Polynomial<CoeffType2, int_<order2> > >,
                      /*com_p1_p2*/ 3> {
  ...
  template <class T, class U> static auto Multiply(const T &t, const U &u) {
    return Simplify(Multiplication<T, U>(t, u));
  }

  template <class T, class U> static auto Add(const T &t, const U &u) {
    return Simplify(Addition<T, U>(t, u));
  }

  template <class T, class U>
  static auto Accumulate(const T &t, const U &u, int_<-1>) {
    return zero;
  }

  template <int index, class T, class U>
  static auto Accumulate(const T &t, const U &u, int_<index> i) {
...
struct Simplification<UnaryMinus<Polynomial<CoeffType, Order> >,
                      /*um_p*/ 0> {
  typedef typename std::conditional<
      std::is_unsigned<CoeffType>::value, std::make_signed<CoeffType>,
      Identity<CoeffType> >::type::type CoeffType2;
  typedef Polynomial<CoeffType2, Order> type;

  static type Combine(UnaryMinus<Polynomial<CoeffType, Order> > u) {
...
template <class Order, class CoeffType1, class CoeffType2, class... Funcs>
struct Simplification<
    Multiplication<Composition<Polynomial<CoeffType1, Order>, Funcs...>,
                   Polynomial<CoeffType2, int_<1> > >,
    /*mult_com_p1_fs_p2_1*/ 0> {
  typedef typename std::common_type<CoeffType1, CoeffType2>::type CoeffType;
  typedef Composition<Polynomial<CoeffType, Order>, Funcs...> type;

  static type
  Combine(Multiplication<Composition<Polynomial<CoeffType1, Order>, Funcs...>,
...
template <template <class...> class Variadic, class Arg1, class Arg2,
          class... Args>
struct Simplification<
    Variadic<Composition<Arg1, Args...>, Composition<Arg2, Args...> >,
    /*mult_f_fvar_com_f1_fs_com_f2_fs*/ 1,
    typename std::enable_if<and_<is_stateless<Args>...>::value &&Simplifies<
        Variadic<Arg1, Arg2> >::value>::type> {
  typedef Composition<SimplifiedType<Variadic<Arg1, Arg2> >, Args...> type;

  static type
  Combine(Variadic<Composition<Arg1, Args...>, Composition<Arg2, Args...> > a) {
...
struct Simplification<
    Addition<Composition<Polynomial<CType, int_<order> >, T>, T>,
    /*add_com_p_f_f*/ 2,
  ...
  template <class A, class B> static auto Add(A a, B b) {
...
template <class CType, int order, class T>
struct Simplification<
    Multiplication<Composition<Polynomial<CType, int_<order> >, T>, T>,
    /*mult_com_p_f_f*/ 2,
    typename std::enable_if<is_stateless<T>::value>::type> {
  static const int new_order = 1 + order;
  typedef Composition<Polynomial<CType, int_<new_order> >, T> type;

  template <class A, class B> static auto Mult(A a, B b) {
    return Simplify(Multiplication<A, B>(a, b));
  }

  static type Combine(
      Multiplication<Composition<Polynomial<CType, int_<order> >, T>, T> a) {
    SIMPLIFY_INFO("Simplifying multiplication of composed "
...
template <template <class...> class Variadic, class CType, int order, class T>
struct Simplification<
    Variadic<T, Composition<Polynomial<CType, int_<order> >, T> >,
    /*var_f_com_p_f*/ 1,
    typename std::enable_if<is_stateless<T>::value>::type> {
  typedef SimplifiedType<
      Variadic<Composition<Polynomial<CType, int_<order> >, T>, T> > type;

  static type
  Combine(Variadic<T, Composition<Polynomial<CType, int_<order> >, T> > a) {
...
template <class CoeffType, class... Functions>
struct Simplification<
...
struct Simplification<Composition<Polynomial<C, O>, Addition<Fs...> >,
                      /*com_p_add_fs*/ 3> {
  typedef Composition<Polynomial<C, O>, Addition<Fs...> > in_type;

  static auto P(C c) { return GetPolynomial(c); }

  template <class T, std::size_t> struct Dummy {
    typedef T type;
  };
  ...
  static auto Mult(C c, Addition<Fs...> a,
    ...
    auto p = P(c);
    Multiplication<decltype(p), typename Dummy<Addition<Fs...>, count>::type...>
    r(P(c), ((void)count, a)...);
    return r;
  }
...
struct Simplification<
    ...
    /*um_com_f_fs*/ 0,
    typename std::enable_if<Simplifies<UnaryMinus<T> >::value>::type> {
  typedef Composition<SimplifiedType<UnaryMinus<T> >, Fs...> type;
...
template <class F> struct Simplification<Composition<F, Zero>, /*com_f_z*/ 1> {
  typedef Polynomial<decltype(std::declval<F>()(0)), int_<1> > type;
...
struct Simplification<Composition<F, Polynomial<C, int_<1> > >,
                      /*com_f_p_1*/ 1> {
  typedef Polynomial<decltype(std::declval<F>()(std::declval<C>())), int_<1> >
  type;
