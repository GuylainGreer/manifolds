...
template <class F, class... Args> struct tuple<F, Args...> {
  static const int num_elements = 1 + sizeof...(Args);
  typedef std::make_integer_sequence<int, 1 + sizeof...(Args)> indices;
  char buffer[1 + sum<sizeof(F), sizeof(Args)...>::value];
  template <class... As> void dummy(As...) {}

  template <int i> struct offset : sum_n<i, sizeof(F), sizeof(Args)...> {};

  template <int... is, class... Fs>
  void fill_with(std::integer_sequence<int, is...>, Fs... fs) {
    dummy(((*((Fs *)(buffer + offset<is>::value)) = fs), 0)...);
  }

  tuple() { fill_with(indices(), F(), Args()...); }

  tuple(F f, Args... args) { fill_with(indices(), f, args...); }

  template <class... Fs, int... is>
  void fill_from(const tuple<Fs...> &t, std::integer_sequence<int, is...> i) {
    dummy(((*(Fs *)(buffer + offset<is>::value) =
                *(const Fs *)(t.buffer + offset<is>::value)),
           0)...);
  }

  tuple(const tuple &t) { fill_from(t, indices()); }
};
...
auto make_same_as(const std::tuple<Ts...> &, Args... args) {
...
template <int n, class... Args> auto &get(tuple<Args...> &t) {
  typedef typename nth<n, Args...>::type type;
  return *(type *)(t.buffer + tuple<Args...>::template offset<n>::value);
}

template <int n, class... Args> const auto &get(const tuple<Args...> &t) {
  typedef typename nth<n, Args...>::type type;
  return *(const type *)(t.buffer + tuple<Args...>::template offset<n>::value);
}

inline bool operator==(tuple<>, tuple<>) { return true; }

template <int n, class Tuple> struct tuple_element;

template <int n, class... Args>
struct tuple_element<n, tuple<Args...> > : nth<n, Args...> {};

template <class T> struct tuple_size : int_<T::num_elements> {};

template <class... As>
struct tuple_size<std::tuple<As...> > : int_<sizeof...(As)> {};

template <class T1, class T2, std::size_t... indices1, std::size_t... indices2>
auto tuple_cat(const T1 &t1, const T2 &t2,
               std::integer_sequence<std::size_t, indices1...>,
               std::integer_sequence<std::size_t, indices2...>) {
  using namespace std;
  return make_same_as(t1, get<indices1>(t1)..., get<indices2>(t2)...);
}
...
template <class T1, class T2> auto tuple_cat(const T1 &t1, const T2 &t2) {
  return tuple_cat(t1, t2, std::make_index_sequence<tuple_size<T1>::value>(),
...
auto remove_element_helper(const Tuple &t,
  ...
  auto r = make_same_as(t, get<befores>(t)..., get<afters + offset>(t)...);
  return r;
}

template <int index, class Tuple> auto remove_element(const Tuple &t) {
  typedef std::make_index_sequence<index> befores;
  typedef std::make_integer_sequence<std::size_t, tuple_size<Tuple>::value -
                                                      index - 1> afters;
...
bool operator==(const tuple<A, As...> &a, const tuple<B, Bs...> &b) {
  return get<0>(a) == get<0>(b) && remove_element<0>(a) == remove_element<0>(b);
}
...
auto insert_element(const Tuple &t, const E &e) {
  return insert_element_helper<index>(
      ...
      std::make_index_sequence<tuple_size<Tuple>::value - index>());
}

template <class Tuple, class E> auto push_back(const Tuple &t, const E &e) {
  return insert_element<tuple_size<Tuple>::value>(t, e);
}
...
FUNC_CHECK_DEF(is_gettable, decltype(get<0>(std::declval<T>())))
...
auto subset_helper(const Tuple &t, int_<start>, int_<size>) {
  ...
  typedef typename boost::mpl::if_c<
      (start + size > tuple_size<Tuple>::value), boost::mpl::range_c<int, 0, 0>,
      boost::mpl::range_c<int, start + size, tuple_size<Tuple>::value> >::type
  second_r;
  typedef typename boost::mpl::copy<
...
auto MakeVariadic(const Tuple &t,
  ...
  return Variadic<typename tuple_element<indices, Tuple>::type...>(
      get<indices>(t)...);
}
