...
using e_if_funcs =
    typename std::enable_if<and_<is_function<Functions>...>::value>::type;

template <class A, class B, class = e_if_funcs<A, B> >
auto operator+(A a, B b) {
  return Simplify(Addition<A, B>(a, b));
}

template <class A, class = e_if_funcs<A> > auto operator-(A a) {
