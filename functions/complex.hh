...
struct Imag : Function<int_<28>, 1, 1>, FunctionCommon<Imag> {
...
struct Phase : Function<int_<28>, 1, 1>, FunctionCommon<Phase> {
...
struct Norm : Function<int_<29>, 1, 1>, FunctionCommon<Norm> {
  using FunctionCommon<Norm>::operator();
  static const bool stateless = true;
  static const ComplexOutputBehaviour complex_output = NeverComplex;

  template <class T> auto eval(T t) const -> decltype(norm(t)) {
    return norm(t);
  }
  ...
  template <class T,
            class = typename std::enable_if<!is_complex<T>::value>::type>
  auto eval(T t) const {
    return t * t;
  }
};
...
struct ImagN : Function<int_<30>, 0, 1>, FunctionCommon<ImagN<Complex> > {
  using FunctionCommon<ImagN>::operator();
  static const bool stateless = true;
  static const ComplexOutputBehaviour complex_output = NeverComplex;

  template <class... Args> auto eval(Args...) const { return Complex(0, 1); }

  template <class T> bool operator==(ImagN<T>) const { return true; }
...
struct Conjugate : Function<int_<32>, 1, 1>, FunctionCommon<Conjugate> {
  using FunctionCommon<Conjugate>::operator();
  static const bool stateless = true;
  static const ComplexOutputBehaviour complex_output = SameAsInput;

  template <class T> auto eval(T t) const { return T(real(t), -imag(t)); }
...
static const ImagN<std::complex<double> > I = ImagN<std::complex<double> >();
}
