...
template <std::size_t rows, std::size_t cols, class CoeffType> struct Matrix {
  static const std::size_t num_rows = rows;
  static const std::size_t num_cols = cols;
  typedef CoeffType coefficient_type;

  Matrix() : coeffs{} {}
  template <class T> Matrix(const std::array<T, rows *cols> &a) : coeffs(a) {}

  CoeffType &Coeff(int row, int col) { return coeffs[row * cols + col]; }

  const CoeffType &Coeff(int row, int col) const {
    return coeffs[row * cols + col];
  }

  auto &GetCoeffs() { return coeffs; }

  const auto &GetCoeffs() const { return coeffs; }

  template <class Type> auto operator()(Matrix<rows, 1, Type> vect) const {
    typename std::common_type<Type, CoeffType>::type result = 0;
  ...
  template <class... Args> auto operator()(Args... args) const {
    ...
    static const std::size_t num_values = sizeof...(Args);
    static_assert(num_values == cols, "Matrix evaluation is matrix "
...
template <std::size_t rows, std::size_t cols, class... Coeffs>
auto GetRowMatrix(Coeffs... args) {
  typedef typename std::common_type<Coeffs...>::type Type;
...
auto GetColMatrix(Coeffs... args) {
  auto matrix = GetRowMatrix<cols, rows>(args...);
  typedef typename std::common_type<Coeffs...>::type Type;
...
template <std::size_t rows, std::size_t cols, class... Args>
auto GetMatrix(Args... args) {
  return GetRowMatrix<rows, cols>(args...);
}

template <std::size_t r, std::size_t c, class T>
std::ostream &operator<<(std::ostream &s, const Matrix<r, c, T> &m) {
  s << "Matrix<" << r << ',' << c << ">{";
...
template <class F> struct IsMatrix : std::false_type {};

template <std::size_t r, std::size_t c, class C>
struct IsMatrix<Matrix<r, c, C> > : std::true_type {};

template <std::size_t rows, std::size_t cols, class CoeffType, class CoeffType2>
bool operator==(const Matrix<rows, cols, CoeffType> &m1,
                const Matrix<rows, cols, CoeffType2> &m2) {
  return std::equal(m1.GetCoeffs().begin(), m1.GetCoeffs().end(),
                    m2.GetCoeffs().begin());
}
}
