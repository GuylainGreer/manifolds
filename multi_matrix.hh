...
namespace manifolds {
  ...
  struct MultiMatrix
  {
    ...
    template <class...Args>
    MultiMatrix(Args ... args):coeffs{{(CoeffType)args...}}{}
  ...
  struct MultiMatrixGroup
  {
    ...
    typedef typename std::common_type<
      typename MMatrices::CoefficientType...
    ...
    struct dimension<N, index, false> :
      dimension<N-dim_helper<index>::dimensions, index+1>{};
    ...
    template <int offset, int ... individual_indices,
	      std::size_t num_indices, int index,
	      class = typename std::enable_if<
		index != sizeof...(MMatrices)>::type>
    auto GetCoeff(std::array<int,num_indices> a,
		  std::integer_sequence<int, individual_indices...>,
  ...
  template <class CoeffType1, int ... sizes1,
	    class CoeffType2, int ... sizes2>
		 ...
		 MultiMatrix<CoeffType, sizes...> a)
  {
    typedef MultiMatrixGroup<
      MMatrices..., MultiMatrix<CoeffType, sizes...>> type;

    return type{insert_element<
	sizeof...(MMatrices)>(b.GetFunctions(), a)};
  }
  ...
  {
    typedef MultiMatrixGroup<
      MultiMatrix<CoeffType, sizes...>, MMatrices...> type;

    return type{insert_element<0>(b.GetFunctions(), a)};
  }

  template <class ... MMatrices1,
	    class ... MMatrices2>
